...
import {getProdukData} from "../../../utils/Service"
...
const Customer = () => {
  ...
  return (
    ...
    <Top produkData={produkData} openAddProduct={openAddProduct} offlineStatus={offlineStatus}  handleOpenPayment={handleOpenPayment} openPayment={openPayment} handleCloseToggle={handleCloseToggle} closeToggle={closeToggle} openDropdown={openDropdown} handleOpenDropdown={handleOpenDropdown}/>
    <div className="flex h-full w-full relative">
        ...
        <div className={`w-full h-auto  bg-white flex sticky top-[80px] shadow-2xl justify-between p-[15px] rounded-md items-center ${openAddProduct ? 'z-[60]':'z-[10]'}`}>
                <p className="font-Roboto font-normal text-[17px]">Product</p>
                <Tippy content={openAddProduct ? 'Close Add Product':'Open Add Product'} arrow arrowType="round" animation="scale" placement="left"  >
                ...
                <div className="bg-white min-w-full overflow-x-auto rounded-md p-[10px] grid grid-flow-dense grid-cols-1 gap-[10px] w-full">
                    <TableProduk produkData={produkData} />     
                </div>
           <Payment openPayment={openPayment} produkData={produkData} Tippy={Tippy}  details={details} handleDetailsDiscount={handleDetailsDiscount}  handleDetails={handleDetails} detailsDiscount={detailsDiscount} cartData={cartData}/>
        </div>
        <Warning openPayment={openPayment} produkData={produkData} handleOpenPayment={handleOpenPayment}/>
    </div>
</div>
  )
}
